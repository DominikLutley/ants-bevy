...
use crate::ants::{FoodInRange, NestInRange};
use crate::food::Food;
use crate::nest::Nest;
...
const EXPECT_NEST_IN_RANGE_MESSAGE: &str = "Ant should contain NestInRange component";

pub fn handle_collisions(
    mut collision_events: EventReader<CollisionEvent>,
    mut query_ants: Query<(&mut WallsInRange, &mut FoodInRange, &mut NestInRange), With<Ant>>,
    query_walls: Query<With<Wall>>,
    query_food: Query<With<Food>>,
    query_nest: Query<With<Nest>>,
) {
    for collision_event in collision_events.iter() {
        match collision_event {
            CollisionEvent::Started(first, second, _) => {
                ...
                if query_nest.contains(*other) {
                    query_ants
                        .get_component_mut::<NestInRange>(*ant)
                        .expect(EXPECT_NEST_IN_RANGE_MESSAGE)
                        .into_inner()
                        .set(*other);
                    continue;
                }
...
fn get_ant_from_collision_event<'a>(
    query_ants: &Query<(&mut WallsInRange, &mut FoodInRange, &mut NestInRange), With<Ant>>,
    first: &'a Entity,
    second: &'a Entity,
) -> (&'a Entity, &'a Entity) {
